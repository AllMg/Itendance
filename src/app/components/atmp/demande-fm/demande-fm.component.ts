...
@Component({
  selector: 'app-demande-fm',
  templateUrl: './demande-fm.component.html',
  styleUrls: ['./demande-fm.component.css']
})
export class DemandeFmComponent implements OnInit {
              ...
              if (result.success) {
                ...
                for (let i = 0; i < this.medical.length; i++) {
                  factureFille.push({
                    ...
                    designation: this.medical[i].designation,
                ...
                this.demandeService.newFactureFille(dataFille).subscribe( resultFille => {
                  if (resultFille.success) {
                    this.toastr.success('Vos médicaments ont été sauvegardé avec success');
                  } else {
                    this.disable = false;
