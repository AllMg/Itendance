    ...
    this.route.params.subscribe((params: Params) => {
      ...
      this.fileService.read(this.idDmdIJ).subscribe(data => {
        if (data.success) {
          ...
          console.log("les pieces", this.pieces)
        } else {
          this.toastr.error('Erreur get Pieces jointe :' + data.msg);
        }
      });

      this.ijService.decompteIj2(this.idDmdIJ).subscribe(data => {
        if (data.success) {
      ...
      this.ijpfService.listchampRequisVal(this.idDmdIJ).subscribe(data => {
        if (data.success) {
          ...
          this.inputs = this.ijpfService.setValidFormDataForDynamicForms2(this.champIjValueList, exception);
        ...
        this.ijpfService.saveToOPforIj(idop, this.idDmdIJ).subscribe(data => {
          if (data.success) {
            ...
            const content = {
              ...
              titre: "Demande d'indemnité journalière deuxième tranche",
            ...
            this.routes.navigate(['/decompte-ij2/' + idop]);
          } else {
  ...
  onPieceJointeClick() {
    ...
    this.ijpfService.changeEtatDemandeIj(dataEtat).subscribe(data => {
      if (data.success) {
        ...
        const notifMessage = "Demande d'indemnite journaliere deuxième tranche ref - " + this.idDmdIJ + " : " + this.pieceJointeMessage;
        const dateToday = this.datePipe.transform(new Date(Date.now()), 'yyyy-MM-dd');
        const content = {
          expediteur: JSON.parse(localStorage.getItem('user')).id_acces,
          destinataire: this.individu.id_individu,
