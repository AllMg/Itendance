...
@Component({
	selector: 'app-rente-ipp',
	templateUrl: './rente-ipp.component.html',
	styleUrls: ['./rente-ipp.component.css']
})
export class RenteIppComponent implements OnInit {
 							...
 							if (data.success) {
 								...
 								this.DynamicsService.getEntite(this.dmd.matricule).subscribe( datlibelle => {
 									if (datlibelle.success) {
 										...
 										this.toastr.error('info benef ATMp error: ' + data.msg);
 									}
 								}); 
 									...
 									if (datlibelle.success) {
 										...
 										this.DynamicsService.getinfopiece(this.refatmp).subscribe(data => {
 											if (data.success) {
 												let infopc = data.msg;
 												console.log(infopc);
 												infopc =  infopc.filter(function(info) {
 													return info.idTypeInfo==26 ;
 												});
 												this.taux_ipp = infopc.valeurInfo;
 											}
 											else {
...
ValidateIJ()
{
	...
	const datas  = {
		...
		"tauxIpp" :this.taux_ipp,
		"idat" :this.iddat,
		"beneflag":this.benef_flag,
		"idEmpl":this.employeur.id_empl,
        "beneficiaire":this.dmd.matricule
	}
	console.log(datas);
	this.show = true;
	this.DynamicsService.SaveRenteIPP(datas).subscribe( result => {
		this.show = false;
		if (result.success) {
			this.DynamicsService.updateEtat(this.refatmp, 13).subscribe( result => {
				if (result.success) {
					this.disabled = true;
					this.toastr.success('La demande Rente IPP/AT n° ' + this.refatmp + ' a été pris en charge');
					const msg = 'Bonjour, nous vous annonçons que votre demande ' +
					'Rente Invalidité Periodique a été pris en charge. Veuillez vous munir des pièces jointes que nous allons vous demander' +
...
PcsNon()
{
	...
	this.DynamicsService.updateEtat(this.refatmp, 11).subscribe( result => {
		...
		if (result.success) {
			this.disabled = true;
			this.toastr.success('La demande Rente IPP/AT n° ' + this.refatmp + ' a été rejeté');
			const msg = 'Bonjour, c\'est avec un sincère regret que nous vous annonçons que votre demande ' +
			'Rente Invalidite  a été rejeté Suite a des piece non conforme. Votre' +
			' numero de demande : ' + this.refatmp + ' numero DAT :  ';
			const content = {
				expediteur: 'System',
				destinataire: this.dmd.matricule,
				titre: 'Demande Rente IPP/AT ',
				message: msg,
				typeNotif: 'DAT',
				dateEnvoi: new Date()
			};
